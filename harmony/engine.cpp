...
public:
  ...
  void deallocate(void *addr) override { return mi_free(addr); }
};
...
Engine Engine::Init(uint32 w, uint32 h, bool enableMSAA, uint64 upfrontMemory,
  ...
  lvk::VkState *vk = HNY_NEW(
      lvk::VkState, lvk::init::Create<lvk::VkSDL, mimalloc_allocator_lvk>(
                        "Harmony Engine", w, h, enableMSAA, enableValidation));

  lvk::LvkIm3dState *im3d = HNY_NEW(lvk::LvkIm3dState, lvk::LoadIm3D(*vk));

  AssetManager *am = HNY_NEW(AssetManager);

  return Engine{std::move(mem), Unique<AssetManager>(am),
                Unique<lvk::VkState>(vk), Unique<lvk::LvkIm3dState>(im3d),
                enableMSAA};
}
